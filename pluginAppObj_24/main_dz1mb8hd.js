function skill_bar_pluginAppObj_24() {
	
	$(document).ready(function(){

		if(isElementInViewport($("#skill_pluginAppObj_24"))){
			drawSkill();
		}
		else{
			firstTimeVisible($("#skill_pluginAppObj_24"), drawSkill);
		}
    ...
    function drawBar(){
		...
		$("#skill_bar_pluginAppObj_24 .count-bar").animate({
			width: getPercentage() + "%"
		}, 50);
    }
    ...
    function drawCircle(){
		...
		var canvas = $("#skill_canvas_pluginAppObj_24").get(0);
		...
		function drawCircleInCanvas(force_value)
		{
			...
			if(true){
				...
				var text;
				if(false){
					text = (force_value ? 310 : Math.floor(degrees/3.6)) + "%";
				}
				else{
					text = (force_value ? 310 : Math.floor(degrees*1/360 * 100));
				}
    ...
    function resizeCircle(){
    	$("#skill_canvas_pluginAppObj_24").attr("width", Math.min(200, $("#pluginAppObj_24").width()));
    	$("#skill_canvas_pluginAppObj_24").attr("height", Math.min(200, $("#pluginAppObj_24").width()));
    }
	...
	function firstTimeVisible(el, callback) {
		if(!isElementInViewport(el)){
			$(window).on('DOMContentLoaded.ftv_pluginAppObj_24 load.ftv_pluginAppObj_24 resize.ftv_pluginAppObj_24 scroll.ftv_pluginAppObj_24', function(){
				if(isElementInViewport(el)){
					$(window).off('.ftv_pluginAppObj_24');
